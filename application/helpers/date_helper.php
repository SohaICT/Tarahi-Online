<?php
//Developed by S.Ghaffari[ghaffari.soroosh@gmail.com]

function normal_view($str, $type = 1)
{
	$arr = explode('-', $str);
	$y = $arr[0];
	switch($arr[1])
	...
	$s = $arr[5];
	return $d." ".$m." ".$y." - ".$h.":".$M.":".$s;
}//End of function normal_view
function standard_view($type = 1)
{
	if($type == 1)
		return implode('-', array(jdate('Y'), jdate('m'), jdate('d'), jdate('H'), jdate('i'), jdate('s')) );
	else 
		return implode('-', array(jdate('Y'), jdate('m'), jdate('d') ) );
}//End of function standard_view
